...
import { Sun, CloudRain, Wind, Thermometer, Droplet, Leaf } from "lucide-react";
...
const weatherIcons: Record<number, JSX.Element> = {
    ...
    3: <CloudRain className="size-20" />, // Overcast
...
export default function WeatherCard() {
    ...
    return (
        <Card className="w-full p-6 bg-blue-500 text-white rounded-lg text-center shadow-lg">
            <h2 className="text-2xl font-semibold">Chiang Rai</h2>

            <div className="flex justify-center items-center my-2">
                {weatherIcons[weather.weather_code] || <CloudRain className="size-20" />}
            </div>
      ...
      <p className="text-sm">{weather.is_day ? "Day" : "Night"}</p> */}
            <div>
                <h1 className="text-4xl font-bold">{Math.round(weather.temperature_2m)}°C</h1>
                <p className="text-lg">{weather.rain > 0 ? "Rainy" : "Sunny"}</p>
            </div>
